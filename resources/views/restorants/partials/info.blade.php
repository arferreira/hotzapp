<div class="pl-lg-4">
    <form method="post" action="{{ route('admin.restaurants.update', $restorant) }}" autocomplete="off" enctype="multipart/form-data">
        ...
        @include('partials.fields',['fields'=>[
            ['ftype'=>'input','name'=>"Nome",'id'=>"name",'placeholder'=>"Nome",'required'=>true,'value'=>$restorant->name],
            ['ftype'=>'input','name'=>"Descrição",'id'=>"description",'placeholder'=>"Descrição",'required'=>true,'value'=>$restorant->description],
            ['ftype'=>'input','name'=>"Endereço",'id'=>"address",'placeholder'=>"Endereço",'required'=>true,'value'=>$restorant->address],
            ['ftype'=>'input','name'=>"Telefone",'id'=>"phone",'placeholder'=>"Telefone",'required'=>true,'value'=>$restorant->phone],
        ]])
        @if(config('settings.multi_city'))
            @include('partials.fields',['fields'=>[
                ['ftype'=>'select','name'=>"Cidade do Estabelecimento",'id'=>"city_id",'data'=>$cities,'required'=>true,'value'=>$restorant->city_id],
            ]])
        @endif
        @if(config('app.ordering'))
            <div class="form-group{{ $errors->has('minimum') ? ' has-danger' : '' }}">
                <label class="form-control-label" for="input-description">{{ __('Quantidade Mínima de Pedido') }}</label>
                <input type="number" name="minimum" id="input-minimum" class="form-control form-control-alternative{{ $errors->has('minimum') ? ' is-invalid' : '' }}" placeholder="{{ __('Informe a quantidade mínima de pedido') }}" value="{{ old('minimum', $restorant->minimum) }}" required autofocus>
        ...
        @if(auth()->user()->hasRole('admin'))
            ...
            <div class="row">
                <div class="col-6 form-group{{ $errors->has('fee') ? ' has-danger' : '' }}">
                    <label class="form-control-label" for="input-description">{{ __('Percentual de Taxa') }}</label>
                ...
                <div class="col-6 form-group{{ $errors->has('static_fee') ? ' has-danger' : '' }}">
                    <label class="form-control-label" for="input-description">{{ __('Taxa Estática') }}</label>
            ...
            <div class="form-group">
                <label class="form-control-label" for="item_price">{{ __('É Destaque?') }}</label>
        ...
        <div class="row">
            <?php
                $images=[
                    ['name'=>'resto_logo','label'=>__('Logo'),'value'=>$restorant->logom,'style'=>'width: 295px; height: 200px;'],
                    ['name'=>'resto_cover','label'=>__('Capa'),'value'=>$restorant->coverm,'style'=>'width: 200px; height: 100px;']
                ]
            ?>
        ...
        <div class="text-center">
            <button type="submit" class="btn btn-success mt-4">{{ __('Salvar Informações') }}</button>
        </div>
    </form>
</div>
